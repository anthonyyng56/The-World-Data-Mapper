...
const RegionSpreadsheetScreen = (props) => {
	...
	const handleDeleteSubregion = async (_id) => {
		const {data} = await DeleteSubregion({ variables: { _id: _id } });
		refetch();
	}
	...
	return (
		<div className="region-spreadsheet-container">
			...
			<div className="region-spreadsheet">
				<RegionSpreadsheetList subregions={subregions} handleUpdateField={handleUpdateField} handleDeleteSubregion={handleDeleteSubregion} />
			</div>
		</div>
	);
};
